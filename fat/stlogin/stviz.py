...
def load_tr_data_mark(crr,val):
    c.execute("SELECT * FROM tr_history  WHERE currency = '%s' ORDER BY trid DESC LIMIT '%s' "%(crr,val))
    dt = c.fetchall()
    conn.commit()
    ...
    data = data[['t_time','Type','Executed price','Executed Qty']]
    data = data.rename(columns={'t_time':'UTC'})
    data['UTC'] = pd.to_datetime(data['UTC'], errors='coerce')
    return data

def fetch_data_mark(crr,val1,val2):
...
def data_viewer(crr,val):
    ...
    dtall = load_tr_data_mark(crr,val)
    val1 = dtall['UTC'].max()
    val2 = dtall['UTC'].min()
    df = fetch_data_mark(crr,val1,val2)


    
    result = df.append(dtall)
    ...
    fig.add_trace(go.Scatter(x=result['UTC'], y=result['Price'],
                    mode='markers',
                    name='raw'))
    fig.add_trace(go.Scatter(x=result['UTC'], y=result['Executed price'],
...
def rawdata_viewer(crr):
    ...
    task = st.selectbox("Choose latest 'x' transaction points from dropdown to view", ["1","5","20","100","200", "500", "1000"])
    if task == "1":
        data_viewer(crr,1)
    if task == "5":
        data_viewer(crr,5)
...
def viewapp(crr):
    ...
    task = st.selectbox("Select Insights", ["Overview","Trades", "Trade-Data Marker","Raw Data"])
    if task == "Overview":
        overviewer(crr)
    if task == "Trades":
        trade_viewer(crr)
    if task == "Trade-Data Marker":
        rawdata_viewer(crr)
    if task == "Raw Data":
        raw_viewer(crr)
