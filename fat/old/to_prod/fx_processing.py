...
def data_proc(thread_name,crr,param,model):
    ...
    while True :
        ...
        if int(days) not in lsts :
            fx_trading(crr, 0,now,dmax,'not',xdate)

        else:
            fx_trading(crr, pmx,now,dmax,'ok',xdate)
...
def fx_trading(crr, pmx,now,dmax,days,xdate):
        ...
        if dfx[1] < 20000:
            ...
            if pmx > 0:
                if dfx[2] < 10:
                    ...
                    conn.commit()
                    nwid = wsid[0]+1
                    #place_order(nwid,pmx,ltsize,crr,'Buy')
        ...
        if ts != None:
            if dfx[2] != 0 :
                ...
                if (ts[2] >= dmax):
                    ...
                    conn.commit()
                    nwid = wsid[0]+1
                    #place_order(nwid,pmx,ltsize,crr,'Sell')
                else:
                    pass
